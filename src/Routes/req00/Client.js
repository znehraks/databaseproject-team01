...
const Req08 = () => {
  ...
  const updateData = (client_name, client_no) => {
    Api.updateClient(client_name, client_no).then((response) => {
      if (response.status === 200) {
        console.log("no err");
        setTrigger(!trigger);
      } else {
        ...
        <Wrapper>
          ...
          <ListContainer>
            <ListItem>
              ...
              <ListItemSpan>발주처이름</ListItemSpan>
                ...
                if ((index >= (page - 1) * 8) & (index < page * 8)) {
                  return (
                    <ListItem
                      onClick={() => {
                        setMode("update");
                        client_name.setValue(item.client_name);

                        setCurrent(item);
                      }}
                    >
                      <ListItemSpan>{item.client_no}</ListItemSpan>
                      <ListItemSpan>{item.client_name}</ListItemSpan>
                      <ListItemSpan>
        ...
        <Wrapper>
          ...
          <ListContainer>
            <SearchContainerColumn>
              발주처이름 <SearchBar {...client_name}></SearchBar>
            </SearchContainerColumn>
          </ListContainer>
          <SubmitButton
            onClick={() => {
              updateData(client_name.value, current.client_no);
              setMode("read");
            }}
          >
            완료
